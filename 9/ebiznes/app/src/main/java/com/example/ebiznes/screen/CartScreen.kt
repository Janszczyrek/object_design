...
import com.example.ebiznes.components.CartEntry
import com.example.ebiznes.components.ProductEntry

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun CartScreen(navController: NavController,
               cartViewModel: CartScreenViewModel = viewModel()) {
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Cart") }
        ...
        content = { padding ->
            LazyColumn(
                ...
                items(cartViewModel.getCart()) { cartItem ->
                    CartEntry(cartItem) { cartViewModel.removeFromCart(it) }
                }
            }
        }
    )
